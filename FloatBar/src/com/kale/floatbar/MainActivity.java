...
public class MainActivity extends PreferenceActivity implements OnPreferenceChangeListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		returnDefault.setOnPreferenceClickListener(new OnPreferenceClickListener() {
			@Override
			public boolean onPreferenceClick(Preference preference) {
				showDialog(MainActivity.this, "恢复默认设置", 
						"确认后悬浮窗将会采用默认的设置","确定","取消");
				return false;
			}
		});
	...
	protected void onResume() {
		...
		List<AccessibilityServiceInfo> list = AccessibilityManagerCompat.
				getEnabledAccessibilityServiceList(manager, AccessibilityServiceInfo.FEEDBACK_ALL_MASK);
		...
		if (!isEnabled) {
			showDialog(this, "激活悬浮窗", "您还没有激活悬浮窗。" +
					"在设置中：系统 → 辅助功能 → 服务 中激活"
	...
	public void showDialog(Context mContext, String title, String msg,
			String positiveMsg,String cancelMsg) {
